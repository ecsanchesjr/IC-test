...
{   // executa o processo de "cortar" as arestas iguais entre os pais, a partir do grafo da união, gerando o Gu'
    for (map<int, CityNode *>::iterator it = unitedGraph.begin(); it != unitedGraph.end(); it++)
    { // percorre todas as entradas do Gu

        vector<CityNode::node> &vec = it->second->getEdges(); // Carrega o vetor com as arestas contidas naquela entrada do mapa

        for (int i = 0; i < vec.size(); i++)
        {   // percorre o vetor de arestas

            int commonEdges{0}; // contagem de vértices iguais, caso contenha dois iguais será executado um "corte"
            ...
            {   // percorre o vetor de arestas
                ...
                {   // verifica igualdade
                    ...
                    if (commonEdges > 1) 
                    {   // caso exista mais do que uma instância do vértice então executa o corte
                     
                        vec.at(i).second = 0;   // Distância entre os vértices = 0 para simbolizar o corte
                        it->second->deleteEdges(last);  // Deleta a primeira instância do vértice
                        it->second->setAccess(true);    // Altera o acesso do vértice (entrada e saída)
                    }
                }
            }
        }
...
{   // a partir do valor passado irá executar uma busca em Gu' para encontrar as partições
    // nodeOne funciona como uma raiz, que será o ponto inicial da busca, achando a partição onde ele está contido
    ...
    {   // enquanto a não for vazio
        ...
        {   // irá percorrer as arestas ligadas

            alreadyVisited = find(idAlreadyVisited.begin(), idAlreadyVisited.end(), n.first) == idAlreadyVisited.end(); // verificar se a aresta não foi visitada

            toVisit = find(nextToVisit.begin(), nextToVisit.end(), n.first) == nextToVisit.end();   // verificar se não está na lista para ser visitada

            cut = n.second != 0;    // verificar se ela não foi cortada

            if (alreadyVisited && toVisit && cut)
            {   // caso todas as condições estejam TRUE

                nextToVisit.push_back(n.first); // lista
            }
        }
    }
...
{   // irá percorrer todo o grafo encotrando as partições presentes nele

    vector<vector<int>> partitions;
    vector<City> cities = citiesList.getCitiesList();

    while (!cities.empty()) 
    {   // enquanto a lista de cidades ainda existir
        ...
        {   // percorre a última partição

            for (int j = 0; j < cities.size(); j++)
            {   // percorre a lista de cidades 

                if (i == cities.at(j).getId())
                {   // se a cidade estiver na última partição
    ...
    for(int i=0; i<partitions.size(); i++){ // retirar partições com tamanho 1
        if(partitions[i].size() == 1){
            partitions.erase(remove(partitions.begin(), partitions.end(), partitions[i]), partitions.end());
            i--;
        }
    }
...
{   // deletar o mapa completamente, desalocando os ponteiros tb
